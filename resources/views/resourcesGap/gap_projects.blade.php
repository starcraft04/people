 ...
 <!-- Selections for the table -->
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel"><!-- Selections for the table -->
      ...
      <div id="fteview">
          <h3>ZZZ-Users view</h3>
        <ul>
          <li>This View to show Projects need consultant with this Practice</li>
        </ul>
      </div>
        <div class="clearfix"></div>
      </div>
    ...
    <!-- ZZZ table -->
    <table id="usersTable" class="table table-striped table-hover table-bordered mytable" width="100%">
          <thead>
            <tr>
              <th>User Practice</th>
              <th>User Name</th>
...
function color_for_month_value(value,td) {
    ...
    activitiesTable = $('#usersTable').DataTable({
      scrollX: true,
      serverSide: true,
      processing: true,
      stateSave: true,
      ajax: {
         url:"{!! route('gapProjectList') !!}",
        type: "POST",
        data: function ( d ) {
          $.extend(d,ajaxData());
          console.log('------------');
      ...
      columns: [
        ...
        { name: 'pr.project_name', data: 'project_name', width:'50px',searchable: true , visible: true},
        @foreach(config('select.available_months') as $key => $month)
