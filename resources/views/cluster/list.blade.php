...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      ...
      <!-- Window content -->
      <div class="x_content">
            <table id="clusterTable" class="table table-striped table-hover table-bordered" width="100%">
                <thead>
                    <tr>
                        ...
                        <th class="last_column">
                          @permission('user-create')
                            <a href="{{ route('clusterFormCreate') }}" class="btn btn-info btn-xs" align="right"><span class="glyphicon glyphicon-plus"> New</span></a>
                          @endpermission
...
@section('script')
    <script>
        ...
        $(document).ready(function() {
            ...
            clusterTable = $('#clusterTable').DataTable({
                serverSide: true,
                processing: true,
                scrollX: true,
                ajax: {
                        url: "{!! route('listOfClustersAjax') !!}",
                        type: "GET",
                ...
                columns: [
                    ...
                    { name: 'clusters.id', data: 'id' , searchable: false , visible: false },
                    { name: 'clusters.name', data: 'name' },
                    {
                        name: 'actions',
                        data: null,
                        sortable: false,
            ...
            $(document).on('click', '.buttonUpdate', function () {
                window.location.href = "{!! route('clusterFormUpdate','') !!}/"+this.id;
            } );

            $(document).on('click', '.buttonView', function () {
                window.location.href = "{!! route('cluster','') !!}/"+this.id;
            } );
            ...
            $(document).on('click', '.buttonDelete', function () {
                ...
                bootbox.confirm("Are you sure want to delete this record?", function(result) {
                    if (result){
                        $.ajax({
                            type: 'get',
                            url: "{!! route('clusterDelete','') !!}/"+record_id,
                            dataType: 'json',
