...
class OtlUploadController extends Controller 
{
	...
	{
        $position = ['main_title'=>'Upload OTL file','second_title'=>'to feed database',
             'urls'=>
                [
		...
		return view('otlupload')->with('position',$position);
	}
	...
	public function postForm(OtlUploadRequest $request)
	{
		...
		if($file->isValid())
		{
            ...
            return redirect('otlupload')->withOk('File uploaded !')->with('results',$results);
		}

		return redirect('otlupload')
